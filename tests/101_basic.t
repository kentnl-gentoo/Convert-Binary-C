...
# $Author: mhx $
# $Date: 2008/04/15 15:37:48 +0200 $
# $Revision: 4 $
# $Source: /tests/101_basic.t $
#
################################################################################
#
...
ok( scalar @warn, 1 );
ok( $warn[0] =~ /^Empty string passed as member expression/ );
...
$p->tag('String', Hooks => { 'pack'   => \&string_pack,
                             'unpack' => \&string_unpack });

$data = pack("N", 4) . "FOO!";

$unpacked = eval { $p->unpack('String', $data) };
ok($@, '');
ok($unpacked, 'FOO!');
